 ...
 * - School Leaders answer every question.
 * - Operations Staff answer only the Facility Condition (FCA) questions.
 */
export function getQuestionsForRole(role: SurveyData["role"]): SurveyQuestion[] {
  if (role === "operations") {
    return SURVEY_QUESTIONS.filter((q) => q.section === "Facility Condition");
  }
