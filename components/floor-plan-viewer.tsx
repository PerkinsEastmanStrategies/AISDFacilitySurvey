...
export function FloorPlanViewer({
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border/60 bg-card p-2.5 shadow-sm">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {availableFloors.length > 1 && onFloorChange && (
            <div
              ...
              className="flex overflow-hidden rounded-lg border border-border bg-background shadow-sm"
            >
        ...
        <div className="flex items-center gap-1.5">
          <Button variant="ghost" size="sm" onClick={() => setZoom((z) => Math.min(z * 1.2, 10))} className="h-9 w-9">
            <ZoomIn className="h-4 w-4" />
          </Button>
          <span className="min-w-[3rem] text-center text-xs font-medium text-muted-foreground">
            {Math.round(zoom * 100)}%
          </span>
          <Button variant="ghost" size="sm" onClick={() => setZoom((z) => Math.max(z / 1.2, 0.1))} className="h-9 w-9">
            <ZoomOut className="h-4 w-4" />
          </Button>
          <div className="mx-1 h-6 w-px bg-border" />
          <Button variant="ghost" size="sm" onClick={fitToWindow} title="Fit to Window" className="h-9 w-9">
            <Maximize className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={fitToWindow} title="Reset View" className="h-9 w-9">
      ...
      <div
        ref={containerRef}
        className="relative flex-1 overflow-hidden rounded-xl border border-border/60 bg-gradient-to-br from-slate-50 to-slate-100 shadow-inner"
      ...
      <p className="flex shrink-0 items-start gap-2 rounded-lg border border-border/60 bg-muted/40 px-3 py-2.5 text-xs leading-relaxed text-muted-foreground">
