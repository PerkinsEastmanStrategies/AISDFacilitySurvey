...
export default function SurveyApp({ defaultSvg }: { defaultSvg: string }) {
  ...
  // are each their own panel (one question per step). Facility Condition
  // questions are grouped by category, so each category is a single panel that
  // shows all of its rating questions together.
  const panels = useMemo<Panel[]>(() => {
    const result: Panel[] = [];
    let i = 0;
    while (i < activeQuestions.length) {
      const q = activeQuestions[i];
      if (q.section === "Facility Condition") {
        // Gather all consecutive questions sharing this category.
  ...
  const renderViewerHeader = (compact = false) => (
    ...
    >
      <div className={compact ? "space-y-1" : undefined}>
        <h2 className="font-sans text-lg font-semibold text-foreground md:text-xl">
          {rightView === "floorplan" ? "Floor Plan" : "Site Map"}
        </h2>
        <p className="text-xs text-muted-foreground md:text-sm">{viewerSubtitle}</p>
      </div>
      <div
        className={
          compact
      ...
      >
        {step === "questions" &&
          ...
          !isRankingPanel &&
          !isTextPanel && (
            <div className="flex overflow-hidden rounded-lg border border-border bg-card shadow-sm" data-tour="annotation-filter">
              <button
                type="button"
                onClick={() => setAnnotationFilterMode("current")}
                className={`flex flex-1 items-center justify-center gap-1.5 px-2.5 py-1.5 text-xs font-medium transition-colors sm:px-3 sm:text-sm ${
        ...
        {step === "questions" && (
          <Badge
            ...
            className="w-fit px-3 py-1.5 text-sm text-white"
          >
            {currentPanel.label}
          </Badge>
        )}
  ...
  const renderMobileViewerWindow = () => {
    ...
    return (
      <div className="mt-4 flex flex-col overflow-hidden rounded-xl border border-border/60 bg-gradient-to-br from-background to-muted/30 p-3 shadow-sm">
        {renderViewerHeader(true)}
        <div
          data-tour="viewer"
          className="mt-2 min-h-[260px] h-[min(45vh,420px)] overflow-hidden rounded-lg border border-border/60 bg-card"
        >
          {renderViewerContent()}
        </div>
      </div>
  ...
  const renderDesktopViewerPanel = () => {
    ...
    return (
      <div className="flex flex-1 flex-col bg-gradient-to-br from-background to-muted/30 p-5">
        {renderViewerHeader(false)}
        <div data-tour="viewer" className="flex-1 overflow-hidden">
          {renderViewerContent()}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      {/* Header */}
      <header className="shrink-0 border-b border-border/60 bg-card/80 px-3 py-2 backdrop-blur-sm sm:px-4 sm:py-3">
        <div className="flex w-full flex-wrap items-center justify-between gap-2 sm:gap-3">
          <div className="flex min-w-0 flex-1 items-center gap-3 sm:gap-5">
            ...
            />
            <div className="hidden h-8 w-px bg-border/60 sm:block md:h-10" />
            <div className="min-w-0">
              <h1 className="truncate font-sans text-base font-semibold tracking-tight text-foreground sm:text-xl">
                Facility Condition and Suitability
              </h1>
              <p className="hidden text-sm text-muted-foreground sm:block">
                Facility Planning &amp; Capital Assessment Survey
              </p>
            </div>
            <div className="hidden h-8 w-px bg-border/60 sm:ml-4 sm:block md:h-10" />
            <div className="hidden text-right sm:block">
              ...
              <Progress value={progressPercent} className="mt-1.5 h-2 w-32 md:w-44" />
            </div>
            {step === "questions" && (
              <>
                <Button
                  ...
                  className="hidden gap-1.5 sm:ml-4 sm:flex"
                >
                  <FileText className="h-4 w-4" />
                  Preview Report
                </Button>
      ...
      <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
        ...
        <div className="flex min-h-0 w-full flex-col border-b border-border/60 bg-card md:w-[32%] md:border-b-0 md:border-r">
            ...
            <div className="p-5">
              {step === "intro" ? (
                ...
                <div className="space-y-4">
                  {/* Panel navigation, grouped by section (accordion) */}
                  <div className="space-y-2" data-tour="progress-nav">
                    {Array.from(
                              ...
                              {panels.map((p, idx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    title={p.label}
                                    className={`flex h-9 min-w-9 items-center justify-center rounded-lg px-2.5 text-sm font-medium transition-all ${
                                      isCurrent
                    ...
                    <>
                      ...
                      {!isRankingPanel && !isTextPanel && (
                        <>
                          ...
                          <div className="rounded-xl border border-border/60 bg-gradient-to-br from-muted/50 to-muted/30 p-4">
          ...
          {/* Navigation Buttons */}
          <div className="shrink-0 border-t border-border/60 bg-card p-5" data-tour="survey-navigation">
            <div className="flex justify-between gap-3">
              ...
              <Button onClick={handleNext} disabled={primaryActionDisabled}>
                {step === "questions" &&
